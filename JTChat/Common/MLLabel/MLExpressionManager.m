...
#define kExpressionLineHeightMultiple 1
...
- (NSAttributedString*)expressionAttributedStringWithAttributedString:(NSAttributedString*)attributedString expression:(MLExpression*)expression {
    ...
    for (NSTextCheckingResult *result in results) {
        ...
        //
        NSString *keyString = expressionAttrStr.string;
        if ([keyString length] >2) {
            keyString = [keyString substringWithRange:NSMakeRange(1, [keyString length] - 2)];
        }
        
        NSString *imageName = expression.expressionMap[keyString];
        if (imageName.length>0) {
            //加个表情到结果中
            NSString *imagePath = [NSString stringWithFormat:@"smiley_%@.png",imageName];//[expression.bundleName stringByAppendingPathComponent:imageName];
            MLTextAttachment *textAttachment = [MLTextAttachment textAttachmentWithLineHeightMultiple:kExpressionLineHeightMultiple imageBlock:^UIImage *(CGRect imageBounds, NSTextContainer *textContainer, NSUInteger charIndex, MLTextAttachment *textAttachment) {
                return [UIImage imageForCurrentBundleWithName:imagePath];
            }];
...
static MLExpression *chatLabelExpression;
...
+ (instancetype)chatLabelExpression{
    if (!chatLabelExpression) {
        chatLabelExpression = [MLExpression expressionWithRegex:@"\\[[a-zA-Z0-9\\u4e00-\\u9fa5]+\\]" plistName:@"emotion" bundleName:@"Expression"];
    }
    return chatLabelExpression;
}
