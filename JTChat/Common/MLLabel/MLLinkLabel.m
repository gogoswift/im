...
REGULAREXPRESSION(PhoneNumerRegularExpression, @"1[3|5|7|8|][0-9]{9}")
...
- (void)setAttributedText:(NSAttributedString *)attributedText withOtherLink:(MLLink *)otherLink{
    //先提取出来links
    self.links = [self linksWithString:attributedText];
    if (!self.links && otherLink) {
        self.links = [[NSMutableArray alloc] initWithObjects:otherLink, nil];
    }
    _activeLink = nil; //这里不能走setter
...
//        [attributedString removeAttributes:[attributes allKeys] range:link.linkRange];
        if ([attributedString length] >= link.linkRange.location + link.linkRange.length) {
...
+ (BOOL)checkTextHasUrlLink:(NSString *)chatContent{
    NSRegularExpression *expression = kURLRegularExpression();
    NSRange textRange = NSMakeRange(0, chatContent.length);
    NSTextCheckingResult *result = [expression firstMatchInString:chatContent options:NSMatchingAnchored range:textRange];
    if (result) {
        return YES;
    }else{
        return NO;
