...
@property (nonatomic, strong) XHEmotionManager *rmtEmotionManager;
...
- (void)addMessage:(id)addedMessageChatModel {
    WEAKSELF
    [self exChangeMessageDataSourceQueue:^{
        NSMutableArray *messages = [NSMutableArray arrayWithArray:weakSelf.messages];
        [messages addObject:addedMessageChatModel];
...
- (void)insertOldMessages:(NSArray *)oldMessages {
    ...
    [self exChangeMessageDataSourceQueue:^{
        ...
        [oldMessages enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            [indexPaths addObject:indexPath];
            CGFloat height = [weakSelf calculateCellHeightWithMessage:[messages objectAtIndex:idx] atIndexPath:indexPath];
            delayOffset.y += height;
        }];
        ...
        [weakSelf exMainQueue:^{
            ...
            //当没有就数据的时候 就设置为nil
            if ([oldMessages count] < 20 && [oldMessages count] > 0) {
                delayOffset.y -= 44;
                weakSelf.messageTableView.tableHeaderView = nil;
            }else if([oldMessages count] == 0){
                weakSelf.messageTableView.tableHeaderView = nil;
            }
...
- (XHShareMenuView *)shareMenuView {
    if (!_shareMenuView) {
        XHShareMenuView *shareMenuView = [[XHShareMenuView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds), CGRectGetWidth(self.view.bounds), 100)];
...
- (NSDictionary *)getEmotions
{
    ...
    if (!ems) {
        NSString *plistPath = [[NSBundle bundleForClass:[XHMessageTableViewController class]] pathForResource:@"index2emotion" ofType:@"plist"];
        ems = [[NSDictionary alloc] initWithContentsOfFile:plistPath];
    }
    return ems;
}

#define custom_emoji_total_count    90

- (XHEmotionManager *)rmtEmotionManager{
    if (!_rmtEmotionManager) {
        _rmtEmotionManager = [[XHEmotionManager alloc] init];
        _rmtEmotionManager.emotionName = [NSString stringWithFormat:@"   "];
        NSMutableArray *emotions = [NSMutableArray array];
        
        NSDictionary *dict=[self getEmotions];
        
        for (NSInteger j = 0; j < custom_emoji_total_count; j ++) {
            XHEmotion *emotion = [[XHEmotion alloc] init];
            emotion.emotionPath = [NSString stringWithFormat:@"[%@]",[dict objectForKey:[NSString stringWithFormat:@"%ld",(long)j]]];
            emotion.emotionConverPhoto = [UIImage imageForCurrentBundleWithName:[NSString stringWithFormat:@"smiley_%ld.png",(long)j]];
            [emotions addObject:emotion];
        }
        _rmtEmotionManager.emotions = emotions;
    }
    return _rmtEmotionManager;
}
...
- (XHVoiceRecordHelper *)voiceRecordHelper {
    if (!_voiceRecordHelper) {
        ...
        _voiceRecordHelper.countDownBlock = ^(NSString *countStr) {
            [weakSelf.voiceRecordHUD showCountDownString:countStr];
        };
...
- (void)initilzer {
    ...
    tableViewFrame.size.height = tableViewFrame.size.height - 64;
	XHMessageTableView *messageTableView = [[XHMessageTableView alloc] initWithFrame:tableViewFrame style:UITableViewStylePlain];
    ...
    // 设置整体背景颜色
    [self setBackgroundColor:[UIColor colorWithHexString:@"0xedeef0"]];
    ...
    // 初始化输入工具条
    XHMessageInputView *inputView = [[XHMessageInputView alloc] initWithFrame:inputFrame withType:[self currentMessageInputViewType]];
...
- (XHMessageInputType)currentMessageInputViewType{
    return XHMessageInputTypeNormal;
}
...
//    }
    ...
    //当算insertoldmessages算offset高度的时候数据还没加到messages数组里，所以最好判断message的属性
...
- (void)didSendMessageWithPhoto:(UIImage *)photo {
    ...
    if ([self.delegate respondsToSelector:@selector(didSendPhoto:fromSender:onDate:)]) {
        photo = [photo resizedImageByWidth:700];
...
- (void)didChangeSendVoiceAction:(BOOL)changed {
    if (changed) {
        ...
        if(self.textViewInputViewType == XHInputViewTypeEmotion){
            //如果是表情键盘，此处选择faceSendButton selected为NO，当再次点击此按钮的时候，能回复表情键盘
...
- (void)didSelecteShareMenuItem:(XHShareMenuItem *)shareMenuItem atIndex:(NSInteger)index {
    ...
    switch (index) {
        case 0: {
            //[self.photographyHelper showMutiImageSelectedOnViewController:self];
            [self showMutiImageSelected];
            break;
        }
        case 1: {
...
//    }
    ...
    NSMutableString *mText = [[NSMutableString alloc] initWithCapacity:100];
    if ([text length] > 0) {
    ...
    [mText appendString:emotion.emotionPath];
    self.messageInputView.inputTextView.text = [mText copy];
...
///////自己的
- (id)messageModelAtIndexPath:(NSIndexPath *)indexPath{
    return nil;
}
...
//    JCHATChatModel *chatModel = [self messageModelAtIndexPath:indexPath];
//    if (!chatModel) {
//        IMLogError(@"JCHATChatModel is nill%@",chatModel);
//        RMTTextMessageTableViewCell *defaultMsgCell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier_Txt forIndexPath:indexPath];
//        [defaultMsgCell configWithChatMsg:nil];
//        return defaultMsgCell;
//    }
//    
//    
//    RMTMessageTableViewCell *cell = nil;
//    if(chatModel.type == kJMSGTextMessage){
//        cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier_Txt forIndexPath:indexPath];
//        [cell configWithChatMsg:chatModel];
//    }else if (chatModel.type == kJMSGImageMessage){
//        cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier_Txt forIndexPath:indexPath];
//    }
//    
//    return cell;
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (object == self.messageInputView.inputTextView && [keyPath isEqualToString:@"contentSize"]) {
        //NSLog(@" contentSize : %@", NSStringFromSize(self.messageInputView.inputTextView.contentSize));
