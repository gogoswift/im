...
- (void)reloadData {
    ...
    NSUInteger pageNum = (numberOfEmotions / (count * kXHEmotionPerRowsCount) + (numberOfEmotions % (count * kXHEmotionPerRowsCount) ? 1 : 0));
    self.emotionPageControl.numberOfPages = pageNum;
    
    shouldShowCounts = pageCount * pageNum;
...
- (void)setup {
    ...
    if (!_emotionSectionBar) {
        XHEmotionSectionBar *emotionSectionBar = [[XHEmotionSectionBar alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.emotionPageControl.frame), CGRectGetWidth(self.bounds), kXHEmotionSectionBarHeight) showEmotionStoreButton:self.isShowSendButton];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XHEmotionManager *emotionManager = [self.dataSource emotionManagerForColumn:self.selectedIndex];
    if (emotionManager.emotions.count > indexPath.row) {
