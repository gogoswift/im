...
+ (NSString *)formattedTimeForChatListFromTimeInterval:(long long)time {
...
//	if (components1.weekday != components2.week) return NO;//<--这句有编译警告
    /**
     通常比较是否为相同的星期，应该都是在月的范围里进行比较的；
     */
...
{
	NSDateComponents *components = [CURRENT_CALENDAR components:DATE_COMPONENTS fromDate:self];
//	return components.week;//这句有编译警告
    /**通常这是第几周，似乎都是以月 为区间 zhy v1.72*/
