...
@interface IMMJPhotoBrowser ()<IMMJPhotoViewDelegate, IMMJPhotoToolbarDelegate>{
    // 滚动的view
...
- (void)photoViewImageFinishLoad:(IMMJPhotoView *)photoView
{
    ...
    IMMJPhoto *photo = [_photos objectAtIndex:_currentPhotoIndex];
    photo.url = photoView.photo.url;
    
    IMMJPhoto *toolbarPhoto = [_toolbar.photos objectAtIndex:_currentPhotoIndex];
...
{
    if (index > 0) {
        IMMJPhoto *photo = _photos[index - 1];
