...
- (void)setPhotos:(NSArray *)photos
{
    ...
    _saveImageBtn.frame = CGRectMake(self.frame.size.width - btnWidth - 20, 0, btnWidth, btnWidth);
    _saveImageBtn.autoresizingMask = UIViewAutoresizingFlexibleHeight;
    [_saveImageBtn setImage:[UIImage imageForCurrentBundleWithName:@"save_icon.png"] forState:UIControlStateNormal];
    [_saveImageBtn setImage:[UIImage imageForCurrentBundleWithName:@"save_icon_highlighted.png"] forState:UIControlStateHighlighted];
...
- (void)saveImage
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        IMMJPhoto *photo = _photos[_currentPhotoIndex];
...
{
    if (error) {
        [JTMBProgressHUD showSuccess:@"保存失败" toView:nil];
    } else {
        IMMJPhoto *photo = _photos[_currentPhotoIndex];
        photo.save = YES;
        _saveImageBtn.enabled = NO;
        [JTMBProgressHUD showSuccess:@"成功保存到相册" toView:nil];
    }
}
