...
- (void)showImage
{
    if (_photo.firstShow) { // 首次显示
        ...
        if (![_photo.url.absoluteString hasSuffix:@"gif"]) {
            ...
            __weak IMMJPhotoView *photoView = self;
            __weak IMMJPhoto *photo = _photo;
            ...
            if (_photo.message.message.msgId) {
                ...
                [((JMSGImageContent *)message.content) largeImageDataWithProgress:^(float percent,NSString *msgId){
                    ...
                    if (error == nil) {
                        //IMLogInfo(@"下载大图 success the image message id is %@",objectId);
                        photo.image = [UIImage imageWithData:data];
                        _imageView.image = [UIImage imageWithData:data];
                        [photoView adjustFrame];
                    } else {
                        //IMLogError(@"下载大图 error");
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([keyPath isEqualToString:@"fractionCompleted"] && [object isKindOfClass:[NSProgress class]]) {
        ...
        //IMLogInfo(@"Progress is %f", progress.fractionCompleted);
